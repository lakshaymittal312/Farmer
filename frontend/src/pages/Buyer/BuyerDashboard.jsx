...
const BuyerDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // 3. Fetch Orders
      const oRes = await orderApi.getBuyerOrders();
      if (oRes.data.success) setOrders(oRes.data.orders || []);

      // 4. Fetch Recommended Products
      const prRes = await productApi.getProducts({ status: 'active', limit: 4 });
  ...
  const cartItemCount = cart?.items?.reduce((acc, i) => acc + (i.quantity || 0), 0) || 0;
