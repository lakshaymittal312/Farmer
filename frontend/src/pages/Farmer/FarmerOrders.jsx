...
const FarmerOrders = () => {
  ...
  const handleUpdateStatus = async (orderId, action) => {
    try {
      let res;
      if (action === 'accept') res = await orderApi.acceptOrder(orderId);
      else if (action === 'reject') res = await orderApi.rejectOrder(orderId, 'Rejected by farmer');
      else if (action === 'process') res = await orderApi.processOrder(orderId);
      else if (action === 'ship') res = await orderApi.shipOrder(orderId);
      else if (action === 'deliver') res = await orderApi.deliverOrder(orderId);
